		...
		<section className={"bg-[#1e1e1e]/30 font-electrohub"}>
			...
			>
				...
				>
					...
					<p className={"mt-3 font-bold text-xl lg:mt-6 lg:text-[28px]"}>
						{item.price} $
					</p>
			...
			>
				{item?.rear_tires && item?.front_tires && (
					...
					>
						<div className={"text-center"}>
							...
							<div className={"text-lg md:text-xl"}>{item.front_tires}</div>
						</div>
						<div className={"text-center"}>
							<div className={"text-gray-500"}>Задние шины</div>
							<div className={"text-lg md:text-xl"}>{item.rear_tires}</div>
						</div>
					</div>
				)}
				<div
				...
				{item?.engine_type && (
					...
					>
						...
						<div className={"text-center"}>
							...
							<div className={"text-lg md:text-xl"}>
								{/*{item.engine_type}*/}
								{item.engine_type === "Гибрид(параллельный)" && (
									<>
										<div>Гибрид</div>
										<div className={"text-xs"}>(параллельный)</div>
									</>
								)}
								{item.engine_type === "Гибрид(последовательный)" && (
									<>
										<div>Гибрид</div>
										<div className={"text-xs"}>(последовательный)</div>
									</>
								)}
								{item.engine_type !== "Гибрид(последовательный)" &&
									item.engine_type !== "Гибрид(параллельный)" && (
				...
				{item?.transmission && item?.privod && (
					...
					>
						<div className={"text-center"}>
							...
							<div className={"text-lg md:text-xl"}>{item.transmission}</div>
						</div>
						<div className={"text-center"}>
							<div className={"text-gray-500"}>Привод</div>
							<div className={"text-center text-lg capitalize md:text-xl"}>
								{item.privod}
							</div>
						</div>
					</div>
				)}
				<div className={"flex flex-col items-center gap-2.5 font-bold text-sm"}>
				...
				>
					...
					<div className={"text-center"}>
						...
						<div className={"text-lg md:text-xl"}>{item.clearance}</div>
					</div>
					<div className={"text-center"}>
						<div className={"text-gray-500"}>Объем багажника</div>
						<div className={"text-lg md:text-xl"}>{item.trunk_capacity}</div>
					</div>
				</div>
			</div>
